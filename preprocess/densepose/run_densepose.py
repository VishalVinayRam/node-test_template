...
class DensePoseEstimatorWrapper:
    def __init__(self, gpu_id: int, model_path: str):
        self.gpu_id = gpu_id
        self.model = DensePoseEstimator(model_path)  
    ...
    def __call__(self, input_image, resolution=384):
        ...
        with torch.no_grad():
            dense_pose_output = self.model(preprocessed_image)

        return dense_pose_output


if __name__ == '__main__':

    model = DensePoseEstimatorWrapper(gpu_id=0, model_path="path/to/model.pt")  
    dense_pose_output = model('./images/bad_model.jpg')

    print(dense_pose_output.shape)  